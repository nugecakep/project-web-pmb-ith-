...
function getJumlahPendaftar() {
    global $conn; // Ambil variabel koneksi dari luar fungsi

    // Query untuk menghitung jumlah pendaftar
    $sql = "SELECT COUNT(id) AS total_pendaftar FROM pendaftar"; // Menghitung jumlah ID

    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();
        return $row['total_pendaftar'];
    } else {
        return 0;
    }
}
